...
public class GrafoFragment extends Fragment implements DialogAddAresta.HelperAddAresta, DialogDeletar.HelperDeletar {
    ...
    private String styleSheet = "graph { fill-color: #CDCDCD; }" +
            ...
            "edge { shape: line; size: 5px; text-size: 40px; padding: 0, 25; fill-mode: plain;}";
    ...
    public void applyAddAresta(String noFrom, String noTo, String peso) {
        ...
        // Se noFrom == noTo não pode adicionar a aresta.
        if (noFrom.equals(noTo)){
            chamaToast(getString(R.string.auto_laco_proibido));
        }else {
    ...
    // Método para enviar as Informações do grafo.
    public void validaEnviar(){
        if (listener != null)
            listener.enviarInformacoes(false, listNos, listArestas, listPeso);
    }
